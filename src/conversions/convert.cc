...
#include "velodyne_pointcloud_oru/convert.h"

#include <velodyne_pointcloud_oru/pointcloudXYZIR.h>
#include <velodyne_pointcloud_oru/organized_cloudXYZIR.h>

namespace velodyne_pointcloud_oru
{
  /** @brief Constructor. */
  Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh, std::string const & node_name):
    data_(new velodyne_rawdata_oru::RawData()), first_rcfg_call(true),
    diagnostics_(node, private_nh, node_name)
  {

    boost::optional<velodyne_pointcloud_oru::Calibration> calibration = data_->setup(private_nh);
    if(calibration)
    {
    ...
    srv_ = boost::make_shared <dynamic_reconfigure::Server<velodyne_pointcloud_oru::
      CloudNodeConfig> > (private_nh);
    dynamic_reconfigure::Server<velodyne_pointcloud_oru::CloudNodeConfig>::
      CallbackType f;
  ...
  void Convert::callback(velodyne_pointcloud_oru::CloudNodeConfig &config,
                uint32_t level)
  {
