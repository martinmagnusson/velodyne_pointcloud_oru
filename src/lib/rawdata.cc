...
inline float SQR(float val) { return val*val; }
  ...
  boost::optional<velodyne_pointcloud_oru::Calibration> RawData::setup(ros::NodeHandle private_nh)
  {
    ...
    if (!private_nh.getParam("calibration", config_.calibrationFile))
      {
        ...
        std::string pkgPath = ros::package::getPath("velodyne_pointcloud_oru");
  ...
  {
    using velodyne_pointcloud_oru::LaserCorrection;
	...
	if(raw_azimuth_diff < 0)//raw->blocks[block+1].rotation - raw->blocks[block].rotation < 0)
      ...
      for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          velodyne_pointcloud_oru::LaserCorrection &corrections = calibration_.laser_corrections[dsr];
